...
La versión 1.3.1 añade el modo **Fondo → Dinámico térmico**: el color representa el estado térmico válido más exigente de CPU/Core Max y GPU, la densidad responde a fluidez/estabilidad de *frame time* y la velocidad a la actividad CPU/GPU, siempre con transiciones suaves y límites configurables. Conserva **Event Intelligence** de 1.3.0: 60 segundos previos, marcas de incidente, puntuación de estabilidad, comparación con la sesión anterior, datos brutos y asistente de privacidad. Toda publicación debe describir el análisis como preliminar y evitar presentarlo como diagnóstico automático definitivo.
