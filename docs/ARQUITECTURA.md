...
El botón OFF tiene cierre redundante: `AlienGamerModeCommand.ps1` escribe una solicitud local y también intenta el evento rápido. El agente recoge cualquiera de las dos vías y ejecuta la misma función `Stop-Monitor` que usa la bandeja, cerrando skin, puente y procesos asociados en orden. La solicitud escrita evita perder la orden cuando Rainmeter y el agente tienen niveles de permisos diferentes. Si no recibe confirmación en tres segundos, `Stop-AlienGamerMode.ps1` ejecuta una limpieza independiente, elimina la solicitud y corrige `agent-state.json`.

Los botones de la skin invocan `AlienGamerModeLauncher.vbs` mediante `wscript.exe //B`; no dependen del complemento `RunCommand` ni crean consolas de PowerShell. El lanzador despacha OFF, grabación e incidentes con estilo de ventana cero. El agente consulta el estado visible de Rainmeter y `%LOCALAPPDATA%\AlienGamerMode\recording-state.json` para actualizar la bandeja. La grabación distingue `recording` de `finalizing`, evitando iniciar otra captura mientras se genera o guarda el reporte.
...
El instalador muestra una sola confirmación final y no inicia el agente hasta que esa ventana y el formulario principal se cerraron. El formulario de instalación conserva PowerShell oculto sin usar `runhidden`, porque esa bandera también ocultaría WinForms. Después, el agente permanente se inicia mediante `wscript.exe` y `AlienGamerModeLauncher.vbs`, con estilo de ventana cero. Se envían dos pulsos idempotentes separados por 1.5 segundos, de modo que la orden no se pierda mientras el agente crea por primera vez su evento local y su icono de bandeja.
...
Las dimensiones se obtienen en coordenadas efectivas de Windows, no en píxeles físicos. Esto evita aplicar dos veces el escalado en configuraciones con DPI mixto; por ejemplo, una pantalla física de 2560×1600 al 150 % se entrega a Rainmeter como 1707×1067. El monitor se resuelve primero mediante su identidad PnP estable y después por su nombre de sesión. La posición se escribe, se fuerza tras activar/refrescar y se reafirma cada cinco segundos para impedir que Rainmeter o una aplicación de pantalla completa reutilicen coordenadas de otro monitor.
...
Desde 1.3.1 también persiste `mode` (`manual` o `thermal`). En modo térmico, `BackgroundAnimator.lua` consulta únicamente medidas ya validadas de Rainmeter. La severidad global es el máximo normalizado de CPU/Core Max y GPU con sus respectivos umbrales; las banderas térmicas fuerzan rojo crítico. Cada segundo conserva una ventana corta de *frame time*, calcula media y desviación, y combina su fluidez/estabilidad con la actividad CPU/GPU para definir una cantidad objetivo. Desde 1.3.2 el modo térmico usa mínimos, máximos, velocidad y tamaño propios, independientes del modo manual. Si GPU o *frame time* indican presión, aplica una reducción progresiva de partículas y velocidad. RGB, densidad y movimiento se interpolan para evitar saltos.
...
Desde 1.4.0, `New-VisualReport` genera junto al libro un HTML autónomo en UTF-8, sin bibliotecas ni conexiones externas. El archivo incluye resumen, puntuación, métricas, cuatro SVG responsivos —FPS, *frame time*, temperaturas y utilización—, marcas de incidentes y contexto protegido. FPS y milisegundos usan escalas independientes. La creación del HTML, el libro y el CSV se controla por separado para conservar la mayor cantidad posible de evidencia si Excel falla.
